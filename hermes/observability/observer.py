...
class TracingObserver(ExecutionObserver):
    ...
    Examples:
        >>> from hermes.observability import TracingObserver, enable_tracing
    ...
    def on_pipeline_start(self, pipeline: Any, context: ExecutionContext) -> None:
        ...
        # Add attributes
        span.set_attribute("hermes.total_rows", context.total_rows)
        span.set_attribute("hermes.session_id", context.session_id)
    ...
    def on_stage_start(self, stage: PipelineStage, context: ExecutionContext) -> None:
        ...
        span.set_attribute("hermes.stage", stage_name)
        span.set_attribute("hermes.processed_rows", context.last_processed_row + 1)
        ...
        if hasattr(stage, "prompt_template") and stage.prompt_template:
            ...
            span.set_attribute("hermes.prompt", prompt_value)
    ...
    def on_stage_complete(
        ...
        if span is not None:
            ...
            span.set_attribute("hermes.rows_processed", context.last_processed_row + 1)
    ...
    def on_pipeline_complete(
        ...
        if span is not None:
            # Add final metrics
            span.set_attribute("hermes.processed_rows", result.metrics.processed_rows)
            span.set_attribute("hermes.failed_rows", result.metrics.failed_rows)
            span.set_attribute(
                "hermes.duration_seconds", result.metrics.total_duration_seconds
            )
            span.set_attribute("hermes.total_cost", float(result.costs.total_cost))
