...
from ceres.core.models import ExecutionResult
from ceres.orchestration.execution_context import ExecutionContext
from ceres.orchestration.observers import ExecutionObserver
...
class TracingObserver(ExecutionObserver):
    ...
    Examples:
        >>> from ceres.observability import TracingObserver, enable_tracing
    ...
    def on_pipeline_start(self, pipeline: Any, context: ExecutionContext) -> None:
        ...
        # Add attributes
        span.set_attribute("ceres.total_rows", context.total_rows)
        span.set_attribute("ceres.session_id", context.session_id)
    ...
    def on_stage_start(self, stage: PipelineStage, context: ExecutionContext) -> None:
        ...
        span.set_attribute("ceres.stage", stage_name)
        span.set_attribute("ceres.processed_rows", context.last_processed_row + 1)
        ...
        if hasattr(stage, "prompt_template") and stage.prompt_template:
            ...
            span.set_attribute("ceres.prompt", prompt_value)
    ...
    def on_stage_complete(
        ...
        if span is not None:
            ...
            span.set_attribute("ceres.rows_processed", context.last_processed_row + 1)
    ...
    def on_pipeline_complete(
        ...
        if span is not None:
            # Add final metrics
            span.set_attribute("ceres.processed_rows", result.metrics.processed_rows)
            span.set_attribute("ceres.failed_rows", result.metrics.failed_rows)
            span.set_attribute(
                "ceres.duration_seconds", result.metrics.total_duration_seconds
            )
            span.set_attribute("ceres.total_cost", float(result.costs.total_cost))
